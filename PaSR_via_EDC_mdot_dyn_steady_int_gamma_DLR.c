...
/*Definition of Coefficient from Jones and Musonge 1998 for Dynamic PaSR*/
#define Cp1JM 1.7     
#define Cp2JM 1.45
#define Cd1JM 1.0
#define Cd2JM 0.9    
...
/*Define parameters*/
real Cmix=1.0;  /*To be modified if counter=1, so if the static Cmix approach is chosen*/
...
#define num_species 3					/* Define number of species to be used for tau c calculation */
char *strs[num_species]={"o2", "h2", "h2o"}; 	/* definition of species that will be used for tau chemistry calculation*/
...
DEFINE_EDC_MDOT(edc_mdot, c, t, mdot, calc_tau, tau)
 {
  ...
  { 
    ...
    /* Check what turbulence model is used in the current simulation */
    /* If a k-omega type model is used: */
    if( GVAR_TURB(rp,kw) )
    	{
      	   /*real Cmu = (rp_kw_easm || rp_kw_wj_bsl_earsm) ? M_kw_easm_Cmu : M_kw_beta_star_inf; */
      	   ted = MAX( C_K(c,t)*C_O(c,t), 1.e-3);   /* This is eps = k*omega */
           ted = MIN(ted, 1.0e8);
	   C_UDMI(c,t,11) = ted;
	}
    /* Else: */
	...
	{
    	ted = MAX(C_D(c,t), 1.e-3); /* Epsilon (turbulent kinetic energy dissipation rate */ 
 ...
 gamma = pow(C_UDMI(c,t,2)/(C_UDMI(c,t,2)+C_UDMI(c,t,3)),0.5);
 C_UDMI(c,t,4)=gamma;
